...
import { NoteCreatedEvent, NoteDeletedEvent, NoteUpdatedEvent } from '../events/NoteEvents';
import { CreateNoteForm, NoteDTO, UpdateNoteForm } from '../models/types';
 ...
 * 实现笔记的业务逻辑、验证和事件发布
 */
export class NewNoteService extends BaseService {
  constructor(
   ...
   * 创建笔记
   */
  async createNote(formData: CreateNoteForm): Promise<NoteDTO> {
    ...
    try {
      ...
      // 发布创建事件
      await this.publishEvent(new NoteCreatedEvent(note.id, { note }));
      ...
      throw new BusinessError('创建笔记失败');
    }
  }
   ...
   * 更新笔记
   */
  async updateNote(id: string, formData: UpdateNoteForm): Promise<NoteDTO | null> {
    // 验证笔记存在
    ...
    try {
      ...
      if (updatedNote) {
        ...
        if (formData.title !== undefined) changes.title = formData.title;
        if (formData.content !== undefined) changes.content = formData.content;
        if (formData.category !== undefined) changes.category = formData.category;
        if (formData.tags !== undefined) changes.tags = formData.tags;
        if (formData.color !== undefined) changes.color = formData.color;
        if (formData.is_pinned !== undefined) changes.is_pinned = formData.is_pinned ? 1 : 0;
        if (formData.is_archived !== undefined) changes.is_archived = formData.is_archived ? 1 : 0;

        await this.publishEvent(new NoteUpdatedEvent(id, { 
          note: updatedNote, 
          changes 
        }));
      ...
      throw new BusinessError('更新笔记失败');
    }
  }
  ...
  async deleteNote(id: string): Promise<boolean> {
    ...
    try {
      ...
      if (success) {
        // 发布删除事件
        await this.publishEvent(new NoteDeletedEvent(id, { noteId: id }));
      }

      return success;
    } catch (error) {
      console.error('删除笔记失败:', error);
      throw new BusinessError('删除笔记失败');
    }
  }
   ...
   * 按分类获取笔记
   */
  async getNotesByCategory(category: string): Promise<NoteDTO[]> {
   ...
   * 按标签获取笔记
   */
  async getNotesByTag(tag: string): Promise<NoteDTO[]> {
   ...
   * 获取置顶笔记
   */
  async getPinnedNotes(): Promise<NoteDTO[]> {
   ...
   * 获取已归档笔记
   */
  async getArchivedNotes(): Promise<NoteDTO[]> {
   ...
   * 归档/取消归档笔记
   */
  async toggleArchive(id: string): Promise<NoteDTO | null> {
    const note = await this.noteRepository.findById(id);
    if (!note) {
      throw new BusinessError('笔记不存在');
    }
   ...
   * 验证笔记数据
   */
  private validateNoteData(data: CreateNoteForm | UpdateNoteForm): void {
    if (!data.title?.trim()) {
      throw new ValidationError('笔记标题不能为空');
    }

    if (data.title.length > 200) {
      throw new ValidationError('笔记标题不能超过200个字符');
    }

    if (data.content && data.content.length > 50000) {
      throw new ValidationError('笔记内容不能超过50000个字符');
    }

    if (data.category && data.category.length > 50) {
      throw new ValidationError('分类名称不能超过50个字符');
    }
   ...
   * 转换为DTO
   */
  private toDTO(note: any, tags: string[]): NoteDTO {
    return {
      id: note.id,
      title: note.title,
