...
class TagDAO extends base_dao_1.BaseDAO {
    constructor() {
        super('tags');
    }
    // 创建新标签
    ...
    async searchByName(name) {
        const result = await this.dbManager.getAllAsync(`SELECT * FROM tags 
    ...
    async findByExactName(name) {
        const result = await this.dbManager.getFirstAsync(`SELECT * FROM tags 
       WHERE LOWER(name) = LOWER(?) AND is_deleted_locally = 0`, [name]);
        return result;
    }
    // 检查标签名称是否已存在
    ...
    async getMostUsedTags(limit = 10) {
        const result = await this.dbManager.getAllAsync(`SELECT t.*, COUNT(nt.note_id) as usage_count
       FROM tags t
    ...
    // 获取未使用的标签
    async getUnusedTags() {
        const result = await this.dbManager.getAllAsync(`SELECT t.*
       FROM tags t
       LEFT JOIN note_tags nt ON t.id = nt.tag_id
    ...
    async getTagUsageCount(tagId) {
        const result = await this.dbManager.getFirstAsync('SELECT COUNT(*) as count FROM note_tags WHERE tag_id = ?', [tagId]);
        return result?.count || 0;
    }
    // 批量创建或获取标签（按名称）
    async createOrGetTagsByNames(tagNames) {
    ...
    async mergeTags(sourceTagId, targetTagId) {
        try {
            return await this.dbManager.withTransaction(async () => {
                ...
                await this.dbManager.runAsync(`UPDATE note_tags 
           SET tag_id = ? 
           WHERE tag_id = ? 
           AND note_id NOT IN (
    ...
    // 获取标签统计信息
    async getStatistics() {
        const totalResult = await this.dbManager.getFirstAsync('SELECT COUNT(*) as count FROM tags WHERE is_deleted_locally = 0', []);
        const total = totalResult?.count || 0;
        const usedResult = await this.dbManager.getFirstAsync(`SELECT COUNT(DISTINCT t.id) as count 
       FROM tags t 
       INNER JOIN note_tags nt ON t.id = nt.tag_id 
       WHERE t.is_deleted_locally = 0`, []);
        const used = usedResult?.count || 0;
        const unused = total - used;
        const mostUsedResult = await this.dbManager.getAllAsync(`SELECT t.name, COUNT(nt.note_id) as count
       FROM tags t
       INNER JOIN note_tags nt ON t.id = nt.tag_id
       WHERE t.is_deleted_locally = 0
