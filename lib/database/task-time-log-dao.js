...
exports.TaskTimeLogDAO = void 0;
const base_dao_1 = require("./base-dao");
class TaskTimeLogDAO extends base_dao_1.BaseDAO {
    constructor() {
    ...
    async findByTaskId(taskId) {
        ...
        try {
            const result = await this.findWhere({ task_id: taskId }, 'created_at DESC');
    ...
    async findActiveLog(taskId) {
        ...
        LIMIT 1
      `;
            const result = await this.dbManager.getFirstAsync(sql, [taskId]);
     ...
     * 查找已完成的时间日志
     */
    async findCompletedLogs(taskId) {
        ...
        ORDER BY start_time DESC
      `;
            const result = await this.dbManager.getAllAsync(sql, [taskId]);
     ...
     * 结束正在进行的时间日志
     */
    async endActiveLog(taskId, endTime) {
     ...
     * 获取某个日期范围内的工作日志
     */
    async findLogsByDateRange(taskId, startDate, endDate) {
        const sql = `
      SELECT * FROM ${this.tableName} 
      WHERE task_id = ? 
        AND start_time >= ? 
      ...
      ORDER BY start_time DESC
    `;
        return await this.dbManager.getAllAsync(sql, [taskId, startDate, endDate]);
    }
    /**
     * 删除任务的所有时间日志（级联删除）
     */
...
exports.TaskTimeLogDAO = TaskTimeLogDAO;
