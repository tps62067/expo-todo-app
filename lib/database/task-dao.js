...
class TaskDAO extends base_dao_1.BaseDAO {
    constructor() {
        super('tasks');
    }
    /**
    ...
    async findByPriority(priority) {
        ...
        return await this.dbManager.getAllAsync(sql, [priorityMap[priority]]);
    }
    /**
     * 根据项目ID查找任务
     */
    async findByProjectId(projectId) {
        return await this.findWhere({ project_id: projectId }, 'sort_order ASC, created_at DESC');
    }
    /**
     * 查找今天的任务
     */
    ...
    `;
        return await this.dbManager.getAllAsync(sql, [startOfDay, endOfDay]);
    }
    /**
     * 查找未完成的任务
     */
     ...
     * 查找已完成的任务
     */
    async findCompletedTasks(limit) {
        const query = `
      SELECT * FROM ${this.tableName}
      WHERE is_deleted_locally = 0
    ...
    `;
        ...
        return this.dbManager.getAllAsync(query, params);
    }
    /**
     * 查找逾期的任务
     */
    ...
    `;
        return await this.dbManager.getAllAsync(sql, [now]);
    }
    /**
     * 查找子任务
     */
    async findSubTasks(parentTaskId) {
        return await this.findWhere({ parent_task_id: parentTaskId }, 'sort_order ASC, created_at ASC');
    }
    /**
     * 查找依赖的任务
     */
    async findDependentTasks(taskId) {
        return await this.findWhere({ depends_on_task_id: taskId }, 'sort_order ASC, created_at ASC');
    }
    /**
     * 查找重复任务
     */
    async findRecurringTasks() {
        return await this.findWhere({ is_recurring: 1 }, 'created_at DESC');
    }
    /**
     * 标记任务为完成
     */
    ...
    `;
        ...
        return await this.dbManager.getAllAsync(sql, [searchPattern, searchPattern, titlePattern]);
    }
    /**
     * 更新任务排序
     */
     ...
     * 批量更新任务排序
     */
    async updateBatchSortOrder(updates) {
     ...
     * 分页查询已完成任务
     */
    async findCompletedTasksPaginated(offset, limit, filters) {
        ...
        const countResult = await this.dbManager.getFirstAsync(countSql, params);
      ...
      LIMIT ? OFFSET ?
    `;
        const tasks = await this.dbManager.getAllAsync(dataSql, [...params, limit, offset]);
     ...
     * 获取已完成任务总数
     */
    async getCompletedTasksCount(filters) {
        let whereClause = 'WHERE is_deleted_locally = 0 AND status = \'completed\'';
        const params = [];
        // 应用筛选条件
        if (filters) {
